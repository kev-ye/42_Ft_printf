...
/*   Updated: 2020/11/28 17:12:56 by kaye             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	parse_int_flag_plus(int i, int count, t_flag flag)
{
	if (i >= 0 && (flag.plus || flag.space) && flag.prec < 0)
	{
		if (flag.plus)
			count += ft_putchar_pf('+');
		else if (flag.space)
...
int			ft_parse_int(int i, t_flag flag)
{
	...
	count = parse_int_flag_plus(tmp_i, count, flag);
